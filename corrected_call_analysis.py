...
print("=" * 100)
print("CORRECTED CALL CENTER ANALYSIS - JANUARY TO AUGUST 2025")
print("Excluding Upload/Assignment Tabs - Only Agent Call Logs")
print("=" * 100)
print()
...
# Tabs to EXCLUDE (upload/assignment tabs)
EXCLUDED_TABS = ['All', 'Cleaned', 'Form Responses 1', 'Form Responses', 'Data', 'Upload', 'Master']

# Function to normalize call status
def normalize_call_status(status):
    if pd.isna(status):
        return None  # Don't count as Unknown if truly missing
...
for month in months:
    ...
    for file in excel_files:
        try:
            ...
            # STRICTLY filter: Only include agent-assigned sheets with CALL STATUS column
            agent_sheets = []
            for sheet in sheet_names:
                # Skip upload/assignment tabs
                if sheet in EXCLUDED_TABS:
                    sheets_skipped.append(f"{file.name} -> {sheet} (Upload tab)")
                    continue
                
                # Check if sheet has CALL STATUS column
                try:
                    df_check = pd.read_excel(file, sheet_name=sheet, nrows=1)
                    has_call_status = any('call status' in str(col).lower() for col in df_check.columns)
                    
                    if has_call_status:
                        agent_sheets.append(sheet)
                        sheets_processed.append(f"{file.name} -> {sheet}")
                    else:
                        sheets_skipped.append(f"{file.name} -> {sheet} (No CALL STATUS column)")
                except:
                    continue
            ...
            for sheet in agent_sheets:
                if 'all in one' in sheet.lower() or 'calls all' in sheet.lower():
            ...
            # Process each contact - ONLY those with actual call status data
            for idx, row in df.iterrows():
                call_status = normalize_call_status(row[status_col])
                
                # SKIP rows with no call status (these are un-called contacts)
                ...
                # Count total calls (only those with status)
                month_total_calls += 1
                ...
                all_contacts_data.append({
                    ...
                    'Interest': normalize_interest(row[interest_col]) if interest_col else None,
                    'Delay_Reason': normalize_delay_reason(row[reason_col]) if reason_col else None
                })
    ...
    if month_total_calls > 0:
        ...
        print(f"\n📊 {month} Summary:")
        print(f"  Unique Contacts Called: {len(month_unique_contacts):,}")
        ...
        print(f"\n  📋 Sheets Processed: {len([s for s in sheets_processed if month in s])}")
        print(f"  ⏭️  Sheets Skipped: {len([s for s in sheets_skipped if month in s])}")

# Generate comprehensive report
print("\n\n")
print("=" * 100)
print("CORRECTED COMPREHENSIVE SUMMARY - JANUARY TO AUGUST 2025")
print("Based ONLY on Agent Call Logs (Excluding Upload Tabs)")
print("=" * 100)
...
print(f"\n📈 OVERALL STATISTICS")
print(f"{'─' * 100}")
print(f"Total Unique Contacts Called: {total_unique:,}")
...
print(f"\n📞 CALL OUTCOME BREAKDOWN (No 'Unknown' from Upload Tabs)")
print(f"{'─' * 100}")
...
if overall_interest_levels:
    ...
    print(f"{'─' * 100}")
    total_interest_recorded = sum(overall_interest_levels.values())
    ...
    interest_gap = total_reached - total_interest_recorded
    interest_gap_pct = (interest_gap / total_reached * 100) if total_reached > 0 else 0
    print(f"{'Not Recorded':<25} {interest_gap:>12,} {interest_gap_pct:>14.1f}%")

print(f"\n⏸️  REASONS FOR DELAYED ENROLLMENT")
print(f"{'─' * 100}")
if overall_delay_reasons:
    total_with_reasons = sum(overall_delay_reasons.values())
    
    if total_with_reasons > 0:
        print(f"{'Reason':<30} {'Count':>12} {'% of Those with Delays':>25}")
        print(f"{'─' * 100}")
        for reason, count in sorted(overall_delay_reasons.items(), key=lambda x: x[1], reverse=True):
...
if total_calls > 0:
    ...
    if overall_interest_levels:
        interested = overall_interest_levels.get('Interested', 0)
        considering = overall_interest_levels.get('Considering', 0)
        interested_pct = (interested / total_reached * 100)
        considering_pct = (considering / total_reached * 100)
    ...
    if overall_delay_reasons and len(overall_delay_reasons) >= 3:
        sorted_reasons = sorted(overall_delay_reasons.items(), key=lambda x: x[1], reverse=True)
...
output_file = base_dir / 'CORRECTED_call_outcomes_analysis.csv'
df_all.to_csv(output_file, index=False)

print(f"\n✅ Corrected call outcomes data saved to: {output_file}")
print(f"\n{'=' * 100}")
